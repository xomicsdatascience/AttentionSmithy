...
class BigBirdAttentionMethod(nn.Module):
    def __init__(
        ...
        self.window_extension_length = window_extension_length
    ...
    def forward(
        ...
        self.initialize_parameters(q, k, global_tokens_query, global_tokens_kv)
        key_blocks, query_blocks, value_blocks = (
            self.reshape_all_tensors_for_block_and_batch_calculations(k, q, v)
        )
        global_query_blocks, sparse_query_blocks, final_outputs_flat = (
            self.split_queries_into_global_and_sparse_blocks(query_blocks)
        )
        global_attention_softmax_weights, global_output_flat = (
            self.calculate_global_query_attention(
                global_query_blocks,
                key_blocks,
        ...
        if self.num_global_query_blocks == self.num_blocks_query:
            return (
                self.reshape_final_outputs(final_outputs_flat, q),
                global_attention_softmax_weights,
            )

        sparse_attention_softmax_weights, sparse_output_flat = (
            self.calculate_sparse_query_attention(
                key_blocks,
                kv_index_table,
                padding_attention_mask,
                sparse_query_blocks,
        ...
        return self.reshape_final_outputs(final_outputs_flat, q), (
            sparse_attention_softmax_weights,
            global_attention_softmax_weights,
        )

    def calculate_global_query_attention(
        self,
        global_query_blocks,
        key_blocks,
        padding_attention_mask,
        ...
        global_attention_scores = self.calculate_global_attention_scores(
            global_query_blocks, key_blocks, padding_attention_mask
        )
        global_attention_softmax_weights = self.calculate_attention_softmax_weights(
    ...
    def calculate_sparse_query_attention(
        ...
        kv_index_table, sparse_attention_scores = (
            self.calculate_sparse_attention_scores(
                kv_index_table, sparse_query_blocks, key_blocks, padding_attention_mask
            )
        )
        sparse_attention_softmax_weights = self.calculate_attention_softmax_weights(
        ...
        sparse_output_flat = self.matmul_sparse_blocks_by_value_blocks(
            kv_index_table, sparse_attention_softmax_weights, value_blocks
        )
        return sparse_attention_softmax_weights, sparse_output_flat

    def initialize_parameters(self, q, k, global_tokens_query, global_tokens_kv):
        ...
        self.raise_variable_errors_if_present()
    ...
    def raise_variable_errors_if_present(self):
    ...
    def set_global_blocks(self, global_tokens, block_size):
        ...
        global_blocks = pad_sequence_with_duplicates(global_blocks)
        return global_blocks.to(global_tokens.device)

    def matmul_sparse_blocks_by_value_blocks(
    ...
    def calculate_sparse_attention_scores(
        self, kv_index_table, sparse_query_blocks, key_blocks, padding_attention_mask
    ):
        if kv_index_table == None:
            kv_index_table = (
                self.create_kv_index_table_for_local_global_random_block_selection().to(
                    key_blocks.device
                )
            )
        if self.max_block_limit != 0:
        ...
        kv_index_table, first_occurrence_mask = determine_first_occurrences(
            kv_index_table
        )
        sparse_attention_scores = self.calculate_all_sparse_attention_scores_from_index(
            sparse_query_blocks, key_blocks, kv_index_table, padding_attention_mask
        )
        sparse_attention_scores = self.pad_duplicate_key_blocks(
    ...
    def pad_duplicate_key_blocks(self, first_occurrence_mask, sparse_attention_scores):
    ...
    def reshape_all_tensors_for_block_and_batch_calculations(self, k, q, v):
        key_blocks, query_blocks, value_blocks = self.reshape_tensors_into_blocks(
            k, q, v
        )
        key_blocks, query_blocks, value_blocks = self.transpose_head_num_and_batch_size(
            key_blocks, query_blocks, value_blocks
        )
        return key_blocks, query_blocks, value_blocks

    def transpose_head_num_and_batch_size(self, key_blocks, query_blocks, value_blocks):
        query_blocks = query_blocks.transpose(0, 1)
        key_blocks = key_blocks.transpose(0, 1)
        value_blocks = value_blocks.transpose(0, 1)
        return key_blocks, query_blocks, value_blocks

    def reshape_tensors_into_blocks(self, k, q, v):
        query_blocks = self.blockify_tensor(
            q, self.num_blocks_query, self.block_size_query
        )
        key_blocks = self.blockify_tensor(
            k, self.num_blocks_kv, self.block_size_kv
        ).transpose(-2, -1)
        value_blocks = self.blockify_tensor(v, self.num_blocks_kv, self.block_size_kv)
        return key_blocks, query_blocks, value_blocks

    def reshape_final_outputs(self, final_outputs_flat, q):
    ...
    def matmul_global_blocks_by_value_blocks(
    ...
    def split_queries_into_global_and_sparse_blocks(self, query_blocks):
    ...
    def blockify_tensor(self, tensor, num_blocks, block_size):
        return tensor.reshape(
            self.batch_size, self.num_heads, num_blocks, block_size, self.head_dim
        )

    def calculate_global_attention_scores(
        ...
        if self.is_causal_masking:
            mask_for_diagonal_blocks, mask_for_future_blocks = (
                self.find_future_and_diagonal_masks_for_global_calculations()
            )
            attention_scores = self.apply_causal_masking(
                attention_scores,
    ...
    def find_future_and_diagonal_masks_for_global_calculations(self):
    ...
    def calculate_attention_softmax_weights(
    ...
    def calculate_all_sparse_attention_scores_from_index(
        ...
        if self.is_causal_masking:
            mask_for_diagonal_blocks, mask_for_future_blocks = (
                self.find_future_and_diagonal_masks_for_sparse_calculations(
                    kv_index_table
                )
            )
            attention_scores = self.apply_causal_masking(
        ...
        if padding_attention_mask != None:
            ...
            self.apply_padding_attention_mask_to_sparse_attention_scores(
                attention_scores, kv_index_table_flat, padding_attention_mask_blocks
            )
        return attention_scores

    def apply_padding_attention_mask_to_sparse_attention_scores(
    ...
    def find_future_and_diagonal_masks_for_sparse_calculations(self, kv_index_table):
        sparse_query_block_indices_offset_by_batch = (
            self.offset_indices_by_batch_for_direct_indexing(
    ...
    def create_kv_index_table_for_local_global_random_block_selection(self):
        local_window_kv_index_table = self.make_local_window_indices()
        random_kv_index_table = self.make_random_indices()
        global_kv_index_tables = self.make_global_kv_index_tables()
        ...
        kv_index_table = self.offset_indices_by_batch_for_direct_indexing(
            kv_index_table
        )
        return kv_index_table

    def offset_indices_by_batch_for_direct_indexing(self, index_table):
    ...
    def make_local_window_indices(self):
        shifts = torch.arange(
            -self.window_extension_length, self.window_extension_length + 1
...
def pad_sequence_with_duplicates(tensors):
