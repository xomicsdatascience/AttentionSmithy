...
def global_tokens_query(batch_size, num_blocks_query, block_size):
    return torch.ones((batch_size, num_blocks_query*block_size))

@pytest.fixture
def global_tokens_kv(batch_size, num_blocks_kv, block_size):
    return torch.ones((batch_size, num_blocks_kv*block_size))

def test__EncoderLayer__works_with_standard_self_attention(query_tensor, numeric_embedding_facade, standard_encoder_layer):
    output = standard_encoder_layer(query=query_tensor, src=query_tensor, numeric_embedding_facade=numeric_embedding_facade)
    assert output.shape == query_tensor.shape

def test__EncoderLayer__works_with_standard_cross_attention(query_tensor, kv_tensor, numeric_embedding_facade, standard_encoder_layer):
    output = standard_encoder_layer(query=query_tensor, src=kv_tensor, numeric_embedding_facade=numeric_embedding_facade)
    assert output.shape == query_tensor.shape

def test__EncoderLayer__works_with_big_bird_self_attention(query_tensor, numeric_embedding_facade, big_bird_encoder_layer, global_tokens_query):
    output = big_bird_encoder_layer(
        query=query_tensor,
        src=query_tensor,
        numeric_embedding_facade=numeric_embedding_facade,
        global_tokens_query=global_tokens_query,
        global_tokens_kv=global_tokens_query)
    assert output.shape == query_tensor.shape


def test__EncoderLayer__works_with_big_bird_cross_attention(query_tensor, kv_tensor, numeric_embedding_facade, big_bird_encoder_layer, global_tokens_query, global_tokens_kv):
    output = big_bird_encoder_layer(
        query=query_tensor,
        src=kv_tensor,
        numeric_embedding_facade=numeric_embedding_facade,
